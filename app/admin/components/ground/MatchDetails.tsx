...
const MatchAnalysis: React.FC<MatchAnalysisProps> = ({ matchId, externalActiveTab, onTabChange }) => {
  ...
  const topBatsmen = useMemo(() => {
    ...
    return [...targetInning.batsmen].sort((a, b) => b.runs - a.runs).slice(0, 4);
  ...
  const allBalls = useMemo(() => {
    return [...inningBalls, ...superoverBalls].map(b => ({
      ...b,
      isSuperOver: !!b.superover_number
    }));
  ...
  const filteredBalls = useMemo(() => {
    ...
    return allBalls.filter(ball => {
      ...
      const matchesPlayerName = !pSearch || 
        ball.batsman_name?.toLowerCase().includes(pSearch) || 
        ball.bowler_name?.toLowerCase().includes(pSearch);
        ...
        {openSections.suspect && (
          <div className="pl-4 pr-2 pb-3 text-sm grid grid-cols-2 gap-2">
            {["chucking", "ball_tampering", "time_wasting", "dangerous_delivery", "beamer"].map(o => (
              <label key={o} className="flex items-center gap-2 text-slate-700">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 accent-violet-600" checked={!!draftFilters[o]} onChange={() => handleToggle(o)}/> 
            ...
            <label className="flex items-center gap-2 text-orange-600 font-bold border-t border-slate-100 pt-2 col-span-2">
              <input type="checkbox" className="w-4 h-4 rounded border-slate-300 accent-orange-500" checked={!!draftFilters['Other Suspect']} onChange={() => handleToggle('Other Suspect')}/> Other
            </label>
          </div>
        )}
      </div>
  ...
  const BASE_MOMENTS = ["wicket", "first_ball_wicket", "hat_trick_wicket", "hat_trick_four", "super_four", "hat_trick_six", "super_six", "five_wicket_haul", "team_all_out"];
  ...
  const getDisplayOutcome = (ball: any) => {
    ...
    if (ball.is_wicket) {
      ...
      return ball.total_runs > 0 ? `${wicketLabel} + ${ball.total_runs} Runs` : wicketLabel;
    }
    if (ball.is_wide) {
      return ball.total_runs > 1 ? `Wide + ${ball.total_runs - 1} Runs` : "Wide";
    }
    if (ball.is_no_ball) {
      return ball.total_runs > 0 ? `No Ball + ${ball.total_runs} Runs` : "No Ball";
    }
    ...
    <div className="flex flex-col gap-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs">
        ...
        <div className="relative z-20">
          <div className="flex flex-wrap items-center gap-3 mb-6 pb-2 z-50">
            ...
            {isFilterOpen && (
              <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-slate-200 p-4 z-[9999]">
                ...
                <div className="mt-4 pt-4 border-t flex gap-2">
                  <button className="flex-1 py-2 text-sm font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-900" onClick={clearAllFilters}>
                    Clear All
                  </button>
                  <button className="flex-1 py-2 text-sm font-bold bg-black text-white rounded-lg hover:bg-slate-800" onClick={(e) => { e.stopPropagation(); setFilters(draftFilters); setIsFilterOpen(false); }}>
                    Apply
                  </button>
                </div>
              </div>
              ...
              {isAppealOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-slate-200 p-4 z-[9999]">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setIsAppealOpen(false); }} className="text-slate-400 hover:text-black transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                    ...
                    {Object.keys(filterOptions).map((displayName) => (
                      <label key={displayName} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded cursor-pointer font-medium border border-transparent text-slate-900">
                        <input 
                          ...
                          onChange={() => setTempAppealFilters(prev => ({ ...prev, [displayName]: !prev[displayName] }))}
                        /> 
                        {displayName}
                      </label>
                    ))}
                  ...
                  <div className="mt-4 pt-4 border-t flex gap-2">
                    <button className="flex-1 py-2 text-sm font-bold border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-900" onClick={() => setTempAppealFilters(prev => Object.keys(prev).reduce((acc, k) => ({...acc, [k]: false}), {}))}>
                      Clear All
                    </button>
                    <button className="flex-1 py-2 text-sm font-bold bg-black text-white rounded-lg hover:bg-slate-800" onClick={() => { setAppealFilters(tempAppealFilters); setIsAppealOpen(false); }}>
                      Apply
                    </button>
                  </div>
                </div>
                ...
                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-xl shadow-xl p-3 max-h-80 overflow-y-auto">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {BASE_MOMENTS.filter(m => m.includes(celebrationSearch.toLowerCase())).map((m) => (
                      <button key={m} onClick={() => { setCelebrationSearch(m); setShowSuggestions(false); }} className="px-2 py-1 bg-slate-100 hover:bg-blue-100 text-slate-700 text-[10px] font-bold rounded-md border border-slate-200 transition-colors">
                        {formatLabel(m)}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    {DYNAMIC_PREFIXES.map(prefix => (
                      <div key={prefix} className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-500 w-20 uppercase">{prefix.replace('_', '')}</span>
                        <div className="flex gap-1 flex-wrap">
                          {MILESTONES.map(m => (
                            <button key={m} onClick={() => { setCelebrationSearch(`${prefix}${m}`); setShowSuggestions(false); }} className="px-2 py-1 bg-orange-50 hover:bg-orange-100 text-orange-600 text-[10px] font-bold rounded border border-orange-200 transition-colors">
                              {m}
                            </button>
                          ))}
                        </div>
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar max-w-full">
                {activeFiltersList.length > 0 ? (
                  activeFiltersList.map((filter) => (
                    <div key={filter.id} className="flex items-center gap-1 bg-slate-100 px-3 py-1 rounded-lg border border-slate-200 text-xs text-slate-700 shadow-sm whitespace-nowrap">
                      {filter.label}
                      <button 
                        onClick={() => {
          ...
          <div className="lg:col-span-2 space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {filteredTimelineKeys.length > 0 ? filteredTimelineKeys.map((key) => (
              ...
              <div className="py-20 text-center text-slate-400 font-medium italic">
                {searchTerm || playerSearchTerm ? `No results found` : "No data available."}
              </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 p-5 sticky top-4 shadow-xs">
              ...
              {activeTab === 'referee' && (
                <div className="flex gap-2 mb-6">
                  <button 
                    ...
                    className="flex-1 py-3 px-3 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 flex flex-col items-center justify-center gap-1.5 transition-all shadow-xs cursor-pointer"
                  >
                    <MessageSquare className="w-5 h-5 text-slate-600" />
                    Add Comment
                  </button>
                  <button 
                    onClick={() => setIsMrJudgementOpen(true)}
                    className="flex-1 py-3 px-3 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 flex flex-col items-center justify-center gap-1.5 transition-all shadow-xs cursor-pointer"
                  >
              ...
              <div className="flex items-center justify-between bg-slate-50/50 p-4 rounded-xl mb-6 border-b border-gray-100">
                <div className="text-center">
                  ...
                  <p className="font-bold text-slate-900">{selectedBallData?.batsman_name || 'R. Sharma'}</p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="font-bold text-slate-900">{selectedBallData?.bowler_name || 'M. Starc'}</p>
                </div>
              </div>
              ...
              {activeTab === 'referee' ? (
                <button 
                  ...
                  className="w-full mt-8 py-3 bg-[#0F1117] text-white rounded-xl font-bold text-xs hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-100 cursor-pointer"
                >
                  Take Action
                </button>
              ) : (
      ...
      {activeTab === 'referee' && (
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs animate-in fade-in duration-300">
          <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-4">
            <h3 className="text-lg font-bold text-slate-900">Actions Taken By Match Referee</h3>
          </div>

          <div className="overflow-x-auto overflow-y-auto max-h-[360px] custom-scrollbar">
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#F8FAFC] text-[11px] font-bold text-slate-500 uppercase tracking-wider sticky top-0 z-10">
                <tr>
                  <th className="py-3 px-4">Over</th>
                  <th className="py-3 px-4">Ball</th>
                  <th className="py-3 px-4">Batter / Bowler</th>
                  <th className="py-3 px-4">Action(s) Taken</th>
...
//   celebrationSearch,
//   searchTerm,
//   playerSearchTerm // Added as dependency
// ]);
...
//                   { l: 'Wicket', v: selectedBallData?.is_wicket ? `Yes (${selectedBallData.wicket_type})` : 'No' },
//                   { l: 'Extra Runs', v: selectedBallData?.extra_runs || 0 },
//                   { l: 'Wide Ball', v: selectedBallData?.is_wide ? 'Yes' : 'No' },
//                   { l: 'No Ball', v: selectedBallData?.is_no_ball ? 'Yes' : 'No' },
//                   { l: 'Bye', v: selectedBallData?.is_bye ? 'Yes' : 'No' },
//                   { l: 'Leg Bye', v: selectedBallData?.is_leg_bye ? 'Yes' : 'No' },
//                   { l: 'Shot Type', v: selectedBallData?.shot_type || 'N/A' },
//                   { l: 'Bowling Length ', v: selectedBallData?.balling_length  || 'N/A' }
//                 ].map((item, i) => (
//                   <div key={i} className="flex justify-between items-center">
//                     <span className="text-sm text-slate-600 font-medium">{item.l}</span>
//                     <span className={`text-sm font-bold ${item.l === 'Wicket' && selectedBallData?.is_wicket ? 'text-red-600' : 'text-slate-900'}`}>{item.v}</span>
